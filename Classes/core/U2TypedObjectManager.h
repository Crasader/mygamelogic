...
TypedObjectManager<T>::~TypedObjectManager()
{
    for (ObjectQueueMap::iterator itr = mObjects.begin();
        itr != mObjects.end();
        ++itr)
    {
...
{
    ObjectQueueMap::const_iterator itr = mObjects.find(type);
    if (itr != mObjects.end())
    {
        const ObjectQueue& queue = itr->second;
        ObjectQueue::const_iterator itr2 = queue.find(name);
        if (itr2 != queue.end())
        {
            return itr2->second;
        }
...
{
    ObjectQueueMap::iterator itr = mObjects.find(obj->getType());
    if (itr != mObjects.end())
    {
        ObjectQueue& queue = itr->second;
        ObjectQueue::iterator itr2 = queue.find(obj->getName());
        if (itr2 != queue.end())
        {
