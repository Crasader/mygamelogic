#include "U2PlatformInformation.h"
...
#endif
#elif (U2_COMPILER == U2_COMPILER_GNUC || U2_COMPILER == U2_COMPILER_CLANG)
    #if U2_ARCH_TYPE == U2_ARCHITECTURE_64
        return true;
    #else
...
static u2uint32 _performCpuid(int query, CpuidResult& result)
{
#if U2_COMPILER == U2_COMPILER_MSVC
#if _MSC_VER >= 1400 
    int CPUInfo[4];
...
#if (U2_COMPILER == U2_COMPILER_GNUC || U2_COMPILER == U2_COMPILER_CLANG)
#if U2_ARCH_TYPE == U2_ARCHITECTURE_32 && U2_CPU == U2_CPU_X86
...
static u2uint32 queryCpuFeatures(void)
{
...
//---------------------------------------------------------------------
static u2uint32 _detectCpuFeatures(void)
{
    u2uint32 features = queryCpuFeatures();

    const u2uint32 sse_features = PlatformInformation::CPU_FEATURE_SSE |
...
static String _detectCpuIdentifier(void)
{
    ...
    if (_isSupportCpuid())
    {
        ...
        if (_performCpuid(0, result))
        {
            ...
            for (u2uint32 i=0x80000000; i<=nExIds; ++i)
            {
...
#elif U2_CPU == U2_CPU_ARM  // U2_CPU == U2_CPU_ARM
...
#elif U2_CPU == U2_CPU_MIPS  // U2_CPU == U2_CPU_ARM
...
//---------------------------------------------------------------------
u2uint32 PlatformInformation::getCpuFeatures(void)
{
    static const u2uint32 sFeatures = _detectCpuFeatures();
    return sFeatures;
}
//---------------------------------------------------------------------
...
#elif U2_CPU == U2_CPU_ARM || U2_PLATFORM == U2_PLATFORM_ANDROID
    pLog->logMessage(
